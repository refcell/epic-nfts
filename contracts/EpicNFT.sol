...
contract EpicNFT is ERC721URIStorage {
    ...
    event EpicMinted(uint256, address);
    ...
    function pickRandomWord(string memory list_name, uint256 tokenId, string[] memory list) public view returns (string memory) {
      // ** Seed the random generator
      uint256 rand = random(string(abi.encodePacked(list_name, Strings.toString(tokenId))));
    ...
    function makeAnEpicNFT() public {
        ...
        string memory desert = desertFolk[random(string(abi.encodePacked("DESERT_FOLK", newItemString))) % desertFolk.length];
        string memory farm = farmFolk[random(string(abi.encodePacked("FARM_FOLK", newItemString))) % farmFolk.length];
        string memory food = foodMakers[random(string(abi.encodePacked("FOOD_MAKERS", newItemString))) % foodMakers.length];
        string memory frozen = frozenLands[random(string(abi.encodePacked("FROZEN_LANDS", newItemString))) % frozenLands.length];
        string memory garment = garmentMakers[random(string(abi.encodePacked("GARMENT_MAKERS", newItemString))) % garmentMakers.length];
        string memory islander = islanders[random(string(abi.encodePacked("ISLANDERS", newItemString))) % islanders.length];
        string memory crafter = crafters[random(string(abi.encodePacked("CRAFTERS", newItemString))) % crafters.length];
        string memory mountain = mountainFolk[random(string(abi.encodePacked("MOUNTAIN_FOLK", newItemString))) % mountainFolk.length];
        ...
        string memory combinedWords = string(abi.encodePacked(desert, " ", farm, " ", food, " ", frozen, " ", garment, " ", islander, " ", crafter, " ", mountain));
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        combinedWords,
                        '", "description": "', combinedWords, ' Edition of Dinosaurs and Caves", "image": "data:image/svg+xml;base64,',
